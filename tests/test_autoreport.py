...
class TestAutoReport(unittest.TestCase):
    ...
    def test_list_fields_on_packs(self):
        ...
        assert ' '.join(field_types) == ' '.join(
            ['TextField', 'FormGPSField', 'FormChoiceFieldWithMultipleSelect']
        )
    ...
    def test_list_fields_from_many_versions_on_packs(self):
        ...
        fields = {
            field.name: field
            for field in fp.get_fields_for_versions(fp.versions.keys())
        }
    ...
    def test_disaggregate_extended_fields(self):
        ...
        stats = [
            (str(repr(field)), field_name, stats_dict)
