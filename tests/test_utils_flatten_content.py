...
def test_internal_method_parsed_sheet_normal():
    ...
    """
    sheet_dicts = _parsed_sheet(
        [['h1', 'h2'], ['r1v1', 'r1v2'], ['r2v1', 'r2v2']]
    )
...
def test_flatten_select_or_other():
    s1 = {
        'survey': [
            {'type': 'select_one_or_other', 'select_from_list_name': 'xyz'}
        ]
    }
    flatten_content(s1, in_place=True)
    row0 = s1['survey'][0]
...
def test_json_hash():
    ...
    assert json_hash(
        OrderedDict([('c', 'x'), ('b', 'y'), ('a', 'z')])
    ) == json_hash(OrderedDict([('a', 'z'), ('b', 'y'), ('c', 'x')]))

    # identical objects match (==)
    assert json_hash({'d': 1, 'e': 2, 'f': 3}) == json_hash(
        {'d': 1, 'e': 2, 'f': 3}
    )
...
def test_order_sheet_names():
    ...
    assert _order_sheet_names(['settings', 'choices', 'survey']) == [
        'survey',
        'choices',
        'settings',
    ]
...
def test_flatten_select_types():
    ...
    assert (
        _flatten_type({'type': 'select_one', 'select_from_list_name': 'xyz'})
        == 'select_one xyz'
    )
    assert (
        _flatten_type(
            {'type': 'select_multiple', 'select_from_list_name': 'xyz'}
        )
        == 'select_multiple xyz'
    )
    assert (
...
def test_translated_col_list():
    assert ['c1', 'c2', 'c3::l1', 'c3::l2'] == translated_col_list(
        ['c1', 'c2', 'c3'], ['l1', 'l2'], ['c3']
    )

    assert ['c1', 'c2', 'c3::l1', 'c3::l2', 'c3'] == translated_col_list(
        ['c1', 'c2', 'c3'], ['l1', 'l2', None], ['c3']
    )

    assert ['c1', 'c2', 'c3'] == translated_col_list(
        ['c1', 'c2', 'c3'], [None], ['c3']
    )
...
def test_flatten_to_spreadsheet_content():
    _e = {
        ...
        'choices': [
            {'list_name': 'xyz', 'name': 'x', 'label': ['X']},
            {'list_name': 'xyz', 'name': 'y', 'label': ['Y']},
            {'list_name': 'xyz', 'name': 'z', 'label': ['Z']},
        ],
        'settings': {
            'xyz': 'abc',
        },
...
def test_flatten_tag_list_util_method():
    ...
    assert flatten_tag_list(['a', 'zz:b'], tag_cols_and_seps={'zz': ' '}) == {
        'tags': 'a',
        'zz': 'b',
    }

    assert flatten_tag_list(['a', 'hxl:b'], tag_cols_and_seps={'hxl': ''}) == {
        'tags': 'a',
        'hxl': 'b',
    }
